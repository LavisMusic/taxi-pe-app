// Receptor de `caja.mirror_cliente` (Caja -> Taxi-PE). Ver
// migración 20260918140000_mirror_cuenta_caja.sql.
//
// Flujo: verificar HMAC -> llamar rpc_webhook_caja_mirror_cliente (que
// hace idempotencia + upsert en 'usuarios'). Toda la lógica de dominio
// vive en el RPC.
//
...
const EXPECTED_EVENT = "caja.mirror_cliente";
...
Deno.serve(async (req) => {
  ...
  const { data, error } = await supabase.rpc("rpc_webhook_caja_mirror_cliente", {
